...
pub(crate) fn normalise_and_truncate(s: &str, max_chars: usize) -> Option<String> {
...
pub(crate) const TITLE_MILESTONES: [usize; 3] = [1, 5, 15];
...
pub(crate) fn extract_cc_title(text: &str) -> Option<String> {
...
pub(crate) fn assistant_text(v: &serde_json::Value) -> Option<String> {
...
pub fn first_user_prompt(path: &Path, max_chars: usize) -> Option<String> {
    ...
    for line in reader.lines().map_while(|r| r.ok()) {
        ...
        if let Some(title) = user_prompt_label(msg, max_chars) {
            return Some(title);
        }
    }
    None
...
/// Renders a user `message` object's text content the same way
/// `first_user_prompt` does (slash-command label, control-char collapse,
/// truncation), or None if it carries no real prompt text. Factored out so
/// `scan_transcript`'s single forward pass can reuse it without re-deriving
/// the skip rules.
pub(crate) fn user_prompt_label(msg: &serde_json::Value, max_chars: usize) -> Option<String> {
...
// Keep in sync: transcript_scan.rs's scan_transcript re-implements this
// override > milestone > first-prompt precedence in a single forward pass.
