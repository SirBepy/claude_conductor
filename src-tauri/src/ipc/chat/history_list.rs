//! `list_history` listing pipeline for the History view.
//!
//! Split out of `history.rs` (which keeps the transcript-replay commands):
//! scan/cache the projects dir, filter, paginate, and expose `list_history`.
//! `collect_history` is re-exported at the old path from `history.rs` since
//! the daemon RPC mirror (`daemon::methods::history`) calls it by that path.
