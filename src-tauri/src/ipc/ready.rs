...
pub fn watch(app: &AppHandle, label: &str, url: &str) {
    registry()
        .lock()
        .unwrap()
        .insert(label.to_string(), Entry { shown: false, attempts: 0 });
    let app = app.clone();
    let label = label.to_string();
    let url = url.to_string();
    tauri::async_runtime::spawn(async move {
        run(app, label, url).await;
    });
}

async fn run(app: AppHandle, label: String, url: String) {
    let mut wait = READY_TIMEOUT;
    loop {
        tokio::time::sleep(wait).await;
        let outcome = {
            let mut reg = registry().lock().unwrap();
            // Unregistered mid-wait means a fresh `watch()` already replaced
            // this entry (window rebuilt) - this stale task has nothing left
            // to do.
            let Some(entry) = reg.get_mut(&label) else { return };
            record_timeout(entry)
        };
        match outcome {
            ...
            TimeoutOutcome::GiveUp => {
                show_error_panel(&app, &label, &url).await;
                return;
            }
        }
    }
...
async fn show_error_panel(app: &AppHandle, label: &str, url: &str) {
    // A heartbeat could have landed between the GiveUp decision and here;
    // never blow away a window that just came up for real.
    if registry().lock().unwrap().get(label).map(|e| e.shown).unwrap_or(true) {
        return;
    }
    let Some(w) = app.get_webview_window(label) else { return };
    let full = app_url(url);
...
mod tests {
    ...
    fn late_heartbeat_after_shown_is_a_no_op() {
        let mut entry = Entry { shown: true, attempts: 0 };
        assert_eq!(record_heartbeat(&mut entry), HeartbeatOutcome::AlreadyShown);
    }
