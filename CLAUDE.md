...
- **Cross-compiling Rust to `aarch64-linux-android` needs four more env vars**, set in the SAME command (PowerShell state does not persist between tool calls). Without them `ring`'s build script cannot find the NDK's clang and the build dies in a C compile, which reads like a broken dependency rather than missing toolchain config: `CC_aarch64_linux_android` and `CXX_aarch64_linux_android` -> `<NDK>/toolchains/llvm/prebuilt/windows-x86_64/bin/aarch64-linux-android28-clang.cmd`, `AR_aarch64_linux_android` -> `llvm-ar.exe` beside it, `CARGO_TARGET_AARCH64_LINUX_ANDROID_LINKER` -> that same clang. The `28` matches `minSdkVersion 28`. Verified 2026-08-08 building `android/` with iroh; iroh's defaults select `tls-ring`, not `aws-lc-rs`, so no extra feature juggling is needed.
